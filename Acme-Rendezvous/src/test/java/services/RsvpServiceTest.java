...
public class RsvpServiceTest extends AbstractTest {
	...
	public void testCreateRsvpDriver() {
		...
		System.out
				.println("===============================================================================================================");
		System.out
				.println("==========================================TEST CREATE RSVP=====================================================");
		System.out
				.println("===============================================================================================================\r");

		Object testingData[][] = {
				{"rendezvous1", "user7", 0, null, "Correcto RSVP."},
				{"rendezvous1", "user8", 0, null, "Correcto RSVP."},
				{"rendezvous2", "user9", 0, null, "Correcto RSVP."},
				{"rendezvous7", "user4", 0, null, "Correcto RSVP."},
				{"rendezvous2", "user7", 1, null, "RSVP con respuestas vacías."},
				{"rendezvous2", "user8", 2, null,
						"RSVP con respuestas con espacios."},
				{"rendezvous2", "user5", 3, null, "RSVP con respuestas nulas."},

				{"rendezvous1", "user1", 0, IllegalArgumentException.class,
						"RSVP en rendezvous con RSVP propio."},
				{"rendezvous1", null, 0, IllegalArgumentException.class,
						"RSVP sin usuario."},
				{"rendezvous1", "manager1", 0, IllegalArgumentException.class,
						"RSVP por manager1."},
				{"rendezvous1", "admin", 0, IllegalArgumentException.class,
						"RSVP por administrador."},
		...
		for (int i = 0; i < testingData.length; i++)
			this.testCreateRsvpTemplate(
					...
					(Class<?>) testingData[i][3], (String) testingData[i][4]);
	}

	protected void testCreateRsvpTemplate(final int rendezvousId,
			final String user, final Integer questionContent,
		...
		try {
			...
			System.out.println("Respuestas antes: "
					+ rsvp.getQuestionsAndAnswers());

			Rsvp saved = this.rsvpService.save(rsvp);

			System.out.println("Respuestas después: "
	...
	public void testFindOneDriver() {
		...
		Object testingData[][] = {
				...
				{"rendezvous1", true, IllegalArgumentException.class,
						"Busqueda de rendezvous1."},
				{"rendezvous1", false, IllegalArgumentException.class,
						"Busqueda con id = 0"}};

		for (int i = 0; i < testingData.length; i++)
			this.testFindOneTemplate(
					super.getEntityId((String) testingData[i][0]),
					(Boolean) testingData[i][1], (Class<?>) testingData[i][2],
					(String) testingData[i][3]);
	}

	protected void testFindOneTemplate(final Integer objectId, Boolean useId,
			final Class<?> expected, final String explanation) {
	...
	public void testDeleteDriver() {
		...
		Object testingData[][] = {
				...
				{"rsvp1", "user1", IllegalArgumentException.class,
						"Borrado de en rendezvous sin rsvp."},
				{"rsvp5", null, IllegalArgumentException.class,
						"Borrado sin actor registrado."},
				{"rsvp6", "user2", IllegalArgumentException.class,
						"Borrado por usuario distinto."},
				{"rsvp6", "manager1", IllegalArgumentException.class,
						"Borrado por manager."}};

		for (int i = 0; i < testingData.length; i++)
			this.testDeleteTemplate(
					super.getEntityId((String) testingData[i][0]),
					(String) testingData[i][1], (Class<?>) testingData[i][2],
					(String) testingData[i][3]);
	}

	protected void testDeleteTemplate(final int rsvpId, final String user,
			final Class<?> expected, final String explanation) {
